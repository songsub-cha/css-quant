# QuantPilot — SoT (Source of Truth) v1.2
...
**한도의 의미**: 모든 한도는 **신규 주문(후보) 차단** 기준이다. 가격 드리프트로 기존 보유가 종목당 비중 상한을 초과해도 강제 청산하지 않는다 — 매도는 전략의 매도 규칙(A6.5)과 (Phase 12) 밴드 리밸런싱이 담당한다.
...
| 7 | 리스크 엔진 | 사전/사후 검증, 차단 로그, 긴급 정지, 시장 충격 차단, **자동 정지 조건**, 누적 손실 킬 스위치, 헬스 대시보드, 외부 heartbeat(D7) |
