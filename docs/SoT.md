# QuantPilot — SoT (Source of Truth) v1.3
...
> **작성일**: 2026-07-11
> **개정**: v1.1 (2026-07-11) — 제품 목적의 2축(투자 정보 모니터링) 명시(A1, A6.11), 비중 복원 리밸런싱 로드맵(A6.5, E2-#11), 알림 인프라 스펙(D2 SMTP/VAPID)·외부 heartbeat(D7) 보강, A6.10 교차 참조 오류 수정
> **개정**: v1.2 (2026-07-12) — 초보자 이해 지원·학습 스펙 신설(A5.11, A6.12, ADR 0010): 용어 도움말 전면 배치, 용어집, 백분위 맥락 해석, 공시 해설, 브리핑 '오늘의 용어', LLM 설명 눈높이
> **개정**: v1.3 (2026-07-12) — 구현 전 완성도 보강: 자산 동일성·티커 재사용(C3), ETF 점수 제외(A6.1, ADR 0011), 상폐 데이터 forward 정책(C2, E2-#16), 알림·관심 목록 스키마(C3), 마스터 키 백업(D5), 잡 미실행 캐치업(D6), 활성 전략 수정(A6.5)·관리종목 보유(A6.6)·현금 모델(A6.7) 정책, 벤치마크 배당 편향 고지(A6.7, E2-#15), 모의-백테스트 정합성 리포트(A6.8), corp_code 매핑(C1), 온보딩 체크리스트(A5.1), 디스크 감시(A5.9), 세전 손익 고지(A6.7.4)
...
오너(단일 사용자)가 매수·매도 알고리즘과 리스크 규칙을 설정하면, AI가 한국 주식 후보를 선별하고 전략 엔진이 백테스트·모의투자·(단계적으로) 실거래까지 지원하는 웹앱. 가격·재무·공시·시장 지표 등 투자 관련 정보는 매 거래일 자동 수집·모니터링되어 AI 점수, 알림, 일일 브리핑(A6.11)으로 오너의 투자 판단을 돕는다. 오너는 투자 초보를 전제로 한다 — 모든 화면·알림의 전문용어는 눈높이 도움말과 함께 제공되어, 시스템을 쓰는 것 자체가 학습이 되도록 한다(A5.11, A6.12). **맥미니 M4에서 Docker로 셀프호스팅**하고 Tailscale로 접속한다. 미국 시장은 검증 후 확장.
...
**MVP**: 오너가 웹앱에서 한국 주식 전략을 만들고, AI 종목 랭킹을 활용해 **백테스트와 모의투자**를 맥미니에서 안정적으로 무인 실행할 수 있는 제품. 공시 감시·일일 브리핑(A6.11) 등 매 거래일 정보 모니터링을 포함한다. (Phase 0~7, A8)

**비목표**:
- 멀티유저 SaaS, 완전 무제한 자동매매, 파생/레버리지/신용/미수, 고빈도/스캘핑, 카피트레이딩, 투자자문/일임, 타인 계좌 운용, 수익률 보장 표현
- **공매도/롱숏** — 롱 온리
- **장중(intraday) 매매** — 일봉 기반 스윙만. 실시간 시세는 감시/알림 용도
- **ML 가격 예측(price oracle)** — 일봉 가격의 신호 대 잡음비 한계. 점수 모델은 A6.10의 비-ML 진화 경로를 따른다
...
- **온보딩 체크리스트**(첫 로그인): ① 위험 성향(보수/중립/공격) 선택 → 리스크 프리셋(A7.2) 적용 ② 알림 채널 연결(푸시 또는 이메일 — 최소 1개 연결 전에는 모의투자 시작 불가, A5.8) ③ "브리핑도 실패 알림도 없는 날 = 이상" 규칙(A6.11)과 긴급 정지 버튼 위치 안내.

### A5.2 종목 탐색
- 시장(KOSPI/KOSDAQ), 자산군(주식/ETF), 섹터로 종목을 필터링한다.
- AI 점수 기준 정렬 목록과 점수 근거(긍정/위험)를 본다.
- 종목 상세: 가격 차트, AI 점수 추이(팩터별), 점수 근거, 핵심 재무 지표, 최근 중요 공시(A6.11), LLM 설명 생성 시각. 모든 지표는 용어 도움말·유니버스 내 상대 위치(백분위 맥락)와 함께 표시된다(A6.12).
...
- 이벤트: 주문 후보 생성(승인형), 주문 체결/실패, 리스크 차단, 긴급·자동 정지 발동, 일일 파이프라인 실패, 모의투자 자동 종료, 대사 불일치, 중요 공시 감지(A6.11), 일일 브리핑(A6.11).
- 채널: 웹 푸시(PWA) + 이메일, 설정에서 관리.
- **셀프호스팅에서 알림은 1차 장애 감지 수단**이므로 승인형 실거래(Phase 9)의 선행 조건이자 무인 운영의 필수 기능. 스택 전체가 죽어 알림 자체가 불가능한 경우는 외부 heartbeat(D7)가 보완한다.

### A5.9 시스템 관리 (오너 = admin)
- 헬스 대시보드: 일일 잡 성공/실패(job_runs), 데이터 신선도, LLM 비용 사용량, **디스크 여유 공간**(임계 미만 시 알림 — 디스크 풀은 DB 정지로 직결), 당일 미실행 잡(D6 캐치업).
...
### A5.11 이해 지원 & 학습 (투자 초보 전제)
- 오너는 화면의 모든 전문용어·지표 라벨에서 쉬운 설명(정의·해석·주의점)을 탭 한 번으로 본다.
- 용어집 화면에서 전체 용어를 검색하고 카테고리별로 훑어볼 수 있다.
- 종목 상세에서는 지표 값이 유니버스에서 어느 수준인지(백분위 맥락)를 함께 본다.
- 중요 공시 알림에는 그 공시가 왜 중요한지 한 줄 해설이 붙고, 일일 브리핑 말미에서 매일 용어 하나를 배운다.
- 스펙은 A6.12.
...
market=KR, **asset_type=STOCK — ETF는 점수 산출에서 제외(아래, ADR 0011)**, is_active, **시총 ≥ 3,000억**, **20일 평균 거래대금 ≥ 10억**, **상장 60일 이상**, 관리종목/투자경고·위험/우선주 제외.
...
**ETF 제외(ADR 0011)**: ETF는 재무제표가 없어 Quality/Value 팩터가 통째로 결측이므로 **AI 점수 산출·랭킹 대상에서 명시적으로 제외**한다 — 결측 보류 규칙에 우연히 의존하지 않는다. ETF는 종목 탐색에서 점수 없이 조회되고, ETF 전략(A7.1 템플릿 ③)은 AI 필터 없이 기술적 조건(변동성·이동평균)만으로 선별한다. 전략 유니버스(A6.5)의 asset_types에서는 ETF 선택 가능.
...
- 시스템 프롬프트 제약: 한국어, 점수·등급·BUY/SELL·매수/매도 단어 금지, 새 숫자 생성 금지(입력 raw 값만 인용), 행동 추천 금지. **투자 초보 눈높이의 쉬운 문장 — 전문용어 첫 사용 시 짧은 풀어쓰기 병기**(A6.12).
...
- 레짐 4분화(Risk-On/Neutral/High-Vol/Risk-Off)는 Phase 11 (A6.10 단계 4).
...
**활성 전략의 수정**: config 수정은 version+1(C3)로 저장되고 **다음 신호 생성부터** 적용된다. 기존 보유 포지션도 다음 실행부터 새 매도 규칙으로 평가한다 — 진입 시점 규칙 고정 아님(규칙을 고치는 의도는 보통 기존 포지션에도 적용하려는 것). 이미 생성된 주문 후보는 생성 당시 config_snapshot(C4)을 따른다.

**리밸런싱 정의**: frequency는 **매수/교체 주기**. 매도 보호 규칙(손절/트레일링/이탈/약화)은 주기와 무관하게 **매 거래일** 평가. 신규 진입·교체(순위 밖 종목 매도 → 상위 매수)는 리밸런스 일(주/월/분기 첫 거래일)에만. 비중 복원(drift 교정)은 MVP 비포함 — 가격 변동만을 이유로 매도하면 회전율·세금 비용이 커진다. Phase 12에서 **밴드 리밸런싱**(목표 비중 대비 상대 ±25% 이탈 시에만 복원, E2-#11)으로 도입을 검토하며, 그때까지 비중 드리프트는 리스크 엔진의 신규 매수 차단으로만 대응한다(A6.6).
...
**한도의 의미**: 모든 한도는 **신규 주문(후보) 차단** 기준이다. 가격 드리프트로 기존 보유가 종목당 비중 상한을 초과해도 강제 청산하지 않는다 — 매도는 전략의 매도 규칙(A6.5)과 (Phase 12) 밴드 리밸런싱이 담당한다. 보유 종목이 **관리종목·투자경고로 지정**되어 유니버스에서 이탈해도 같은 원칙: 강제 청산 없음, 신규 매수만 차단. 지정 사실은 공시 감시(A6.11)가 즉시 알리고 종목 상세에 플래그되며, 매도 여부는 매도 규칙과 오너 판단에 맡긴다.
...
- 현금 모델: **매도 체결 즉시 현금 가용**(당일 재매수 가능 — 한국 시장의 결제 전 매매 관행과 일치). 백테스트·모의·실거래 동일. 예수금 이자는 미반영.
...
- Survivorship: 상폐 종목 포함 (`assets.delisted_at`). 단, 백필 소스 한계로 과거 구간은 편향이 잔존할 수 있다(C2 forward 정책, E2-#16) — 결과 화면에 해당 구간을 표시한다.
- **과거 시점 유니버스**: 백테스트 당시의 시총·관리종목 스냅샷(asset_factors)으로 필터 — 현재 상태로 과거를 거르지 않는다.
- 수정주가 일관 사용.

**3) 결과 지표**: 누적 수익률, CAGR, MDD, 변동성, 샤프, 승률, **payoff ratio**, 거래 횟수, 평균 보유 기간, 회전율, 월별 수익률, 종목별 기여도, 수수료/세금 합계, 레짐별 성과 분해, **벤치마크(KOSPI/KOSDAQ) 대비 초과수익·베타**, **체결 실패 통계**. 추세추종은 승률보다 payoff ratio로 평가하도록 안내. **벤치마크 편향 고지**: 전략 수익률은 수정주가로 배당이 반영되는 반면 KOSPI/KOSDAQ은 배당 미포함 가격지수라, 초과수익이 배당수익률만큼(연 1~2%p) 구조적으로 유리하게 보인다 — 결과 화면에 고지하고, 총수익지수(TR) 전환은 E2-#15.

**4) 안내 문구** (백테스트 결과·모의투자 화면 상단 상시):
> 과거 수익률은 미래 수익률을 보장하지 않습니다. 데이터 한계, 슬리피지, 세금으로 실제와 다를 수 있습니다. 손절 주문은 갭 하락 시 설정값보다 큰 손실로 체결될 수 있습니다. 표시되는 손익은 배당소득세 등 보유 관련 세금을 반영하지 않은 세전 기준입니다.
...
- **모의-백테스트 정합성 리포트 (Phase 7)**: "백테스트·모의는 동일 정책"이라는 전제를 검증한다 — 월 1회 활성 paper 포트폴리오의 운용 구간을 동일 config·기간으로 백테스트 재실행해 일별 포지션·체결가를 비교. 괴리 = 구현 버그 신호 → 알림 + 원인 규명 전 실거래 전환 보류(A6.9.4 전환 기준의 입력).
...
### A6.11 정보 모니터링 — 공시 감시 & 일일 브리핑

> 제품 목적의 두 번째 축: 투자 관련 정보를 상시 수집·모니터링해 오너의 투자 판단을 돕는다. 단, **비정형 정보(공시·뉴스)는 MVP에서 점수·신호·주문에 자동 개입하지 않는다** — 표시·알림 전용 (원칙 7과 백테스트 재현성 유지). 전략 입력으로의 활용(이벤트 플래그)은 Phase 12에서 검토.

**1) 공시 감시 (Phase 6, 최소형)**
- DART 공시목록 API로 **보유(모의 포함)·관심 종목**의 수시공시를 일 2회 수집(D6): 17:35(당일분) + 익일 08:00(직전 수집 이후 접수분 — 장 마감 후 악재 공시를 후보 만료 08:30 전에 잡는다). `disclosure_events`에 저장(C3), `dart_rcept_no`로 멱등.
- **중요 공시 화이트리스트**(설정 테이블, E2-#12 시드): 유상/무상증자, 감자, 합병·분할, 영업정지, 감사의견 관련, 불성실공시법인 지정, 횡령·배임, 소송, 매출·손익 30% 이상 변동, 관리종목·투자주의환기 지정, 상장폐지 관련.
- 화이트리스트 매칭 시 즉시 알림(A5.8) + 종목 상세 플래그(A5.2). 승인형 모드에서는 대기 중인 주문 후보의 승인/거절 판단 근거가 된다. 알림·플래그에는 공시 유형별 한 줄 해설(A6.12)이 함께 붙는다 — 오너가 알림만 보고 "왜 중요한지"를 알 수 있어야 한다.
- 전 종목 공시 스트림·뉴스 수집·전략 연동은 Phase 12.

**2) 일일 브리핑 (Phase 6)**
- 파이프라인 완료 후 17:45(D6)에 푸시+이메일 1건으로 발송: ① 파이프라인·데이터 신선도 상태 ② 레짐·시장 충격 여부 ③ 포트폴리오 요약(손익, 리스크 이벤트) ④ 신규 주문 후보 ⑤ AI 점수 급변(±10점) 종목 ⑥ 당일 중요 공시(한 줄 해설 포함) ⑦ 오늘의 용어(A6.12).
- 파이프라인 실패 시 브리핑 대신 실패 알림(A6.4)이 발송된다. **"브리핑도 실패 알림도 없는 날 = 시스템 침묵 = 이상"** — 이 규칙을 온보딩에서 안내하고, 침묵 자체는 외부 heartbeat(D7)가 감지한다.

### A6.12 초보자 이해 지원 — 용어 도움말 & 학습 요소

> 오너는 투자 초보다(A1). 시스템이 보여주는 숫자를 오너가 이해하지 못하면 승인형 모드의 승인/거절도, 전략 설정도 형식적이 된다 — **이해 지원은 부가 기능이 아니라 안전 기능**이다. 도움말 텍스트는 **검수된 정적 콘텐츠만 사용하고 LLM 실시간 생성을 금지**한다(ADR 0010): 틀린 설명은 설명 없음보다 해롭고, 정적 콘텐츠는 비용이 0이다. LLM으로 초안을 만들어 오너 검수 후 수록하는 것은 제작 워크플로로 허용.

**1) 용어집 (Phase 3)**
- 단일 소스: 저장소 내 `glossary.ko.yaml` — git으로 버전 관리, 변경은 코드 리뷰로 검수. DB 테이블 아님(읽기 전용 콘텐츠). 백엔드가 로드해 API로 프론트에 제공하고, 알림·브리핑·이메일 문구 생성 시에도 동일 항목을 참조한다(화면과 이메일의 설명 불일치 방지).
- 항목 스키마: key, 용어(한/영), category(팩터, 재무지표, 성과지표, 거래·주문, 리스크, 공시, 시장·레짐), definition(쉬운 정의 1~2문장), interpretation(높으면/낮으면), caution?(함정 — 예: 낮은 PER의 밸류 트랩, 추세추종은 승률이 아닌 payoff ratio로 평가), in_system?(이 시스템에서의 역할 — "Value 팩터 입력", "매도 규칙 2번"), direction?(higher_is_better | lower_is_better | neutral — 백분위 맥락 문구의 해석 방향), related?(관련 용어 key).
- 초기 수록 범위: 이 SoT에 등장하는 용어 전수(팩터 5종과 구성 지표, 백테스트 결과 지표, 매도 규칙·리스크 한도 용어, 공시 화이트리스트, 레짐·충격)를 시드로 약 60~80항목 (E2-#14).
- 용어집 화면: 검색 + 카테고리 브라우징 (A5.11).

**2) 인라인 도움말 (Phase 3~)**
- 프론트 공통 컴포넌트 하나(`<Term>`): 용어 라벨에 점선 밑줄, **탭/클릭으로 팝오버** 열림(모바일 PWA 필수라 hover 전용 금지 — A5.10). 팝오버 = 정의·해석·주의 + "용어집에서 더 보기".
- 적용 범위: 종목 상세, 랭킹, 백테스트 결과, 전략 편집 폼 라벨, 리스크 이벤트, 레짐 배지, 알림 상세 등 전문용어가 노출되는 모든 화면.
- **DoD 규칙: 전문용어·지표를 노출하는 새 화면은 해당 용어의 용어집 연결(필요 시 신규 항목 추가 포함)까지가 완료 기준이다** — "적극적 도움말"이 초기 화면에서 멈추지 않게 하는 장치.

**3) 백분위 맥락 해석 (Phase 3~)**
- 종목 상세의 지표 팝오버에는 정의와 함께 **그 종목의 유니버스 내 상대 위치**를 병기: "이 종목의 PER은 유니버스 하위 12% — Value 관점에서 싼 편". 점수 파이프라인이 이미 계산한 백분위(A6.1 단계 3)를 표시만 한다 — 새 계산·LLM 개입 없음(원칙 7 유지). 문구의 방향(싼/비싼, 좋은/나쁜)은 용어집 direction 필드를 따른다.

**4) 학습 요소 (Phase 6)**
- 중요 공시 해설: A6.11 알림·플래그에 공시 유형별 한 줄 해설 삽입 — 예: "유상증자: 신주 발행으로 기존 주주 지분 희석, 통상 단기 악재로 해석되나 목적(시설투자/채무상환)에 따라 다름".
- 오늘의 용어: 일일 브리핑 ⑦(A6.11) — 당일 이벤트(발동한 매도 규칙, 중요 공시, 레짐 전환)와 관련된 용어를 우선 선택, 없으면 미노출 용어 순환. 매일 브리핑을 읽는 것만으로 학습이 누적된다.
- LLM 설명 눈높이: A6.1 단계 5의 프롬프트 제약("전문용어 첫 사용 시 풀어쓰기")이 담당.

DB 스키마·환경변수·크론 추가 없음 — 정적 파일과 기존 잡(send_daily_briefing 등)에 편승한다.
...
| 3 | AI 점수 파이프라인 | 정규화 → 레짐 감지 → 레짐별 가중 합산 → LLM 설명, 랭킹 화면, 레짐 배지, 관심/제외, **데이터 품질 게이트**, **용어 도움말 v1(공통 컴포넌트 + 용어집 화면 + 팩터·점수 용어, A6.12)** |
...
| 6 | 모의투자 + 정보 모니터링 | PaperBroker, 일일 잡, 포트폴리오 화면, **알림 v1(웹 푸시+이메일)**, 체결가 재현성, **공시 감시 최소형 + 일일 브리핑(A6.11)** |
| 7 | 리스크 엔진 | 사전/사후 검증, 차단 로그, 긴급 정지, 시장 충격 차단, **자동 정지 조건**, 누적 손실 킬 스위치, 헬스 대시보드, 외부 heartbeat(D7), 모의-백테스트 정합성 리포트(A6.8) |
...
| 12 | 고도화 | 실시간 시세(개장 전 충격 검사 포함), 리포트, 뉴스 수집·공시 이벤트의 전략 플래그화(A6.11 확장), 밴드 리밸런싱(비중 복원, E2-#11), 모의 배당 처리(E2-#13), CA 자동 처리 |
...
| 재무제표 | DART OpenAPI | FDR | 분기, 공시일 저장. corp_code↔티커 매핑 파일 주기 갱신(신규 상장 반영) |
...
| 수시공시 목록 | DART OpenAPI | — | 보유·관심 종목, 일 2회 (A6.11) |
...
- **백필 vs 증분 분리**: 히스토리 백필(최소 2015년~, 상폐 포함 목표)은 1회성 잡, 이후 일 1회 증분. 백필 범위 = 백테스트 가능 기간.
- **상폐 종목 한계와 forward 정책**: 무료 소스(pykrx/FDR)는 이미 상폐된 종목의 과거 시세 제공이 제한적이다 — 백필 구간의 survivorship 편향 잔존 가능성을 인지된 한계로 둔다(E2-#16). 대신 **운영 개시 후 수집분은 상폐되어도 삭제하지 않고 축적**해, 시간이 갈수록 편향 없는 구간이 늘어난다. 편향 가능 구간은 백테스트 결과의 데이터 시작일 표시(A6.7.6)와 함께 안내.
...
- **assets**: ticker, name, market enum(KR), asset_type enum(STOCK, ETF), exchange enum(KOSPI, KOSDAQ), sector?, currency default 'KRW', is_active, **listed_at?**, delisted_at?, **is_managed, is_alert** (현재 상태 — 과거 시점은 asset_factors 스냅샷). **자산 동일성 = 티커 + 상장기간**: KRX 종목코드는 상폐 후 재사용될 수 있으므로 UNIQUE는 활성 종목에만 적용(partial index: ticker, market WHERE is_active), 같은 티커의 재상장은 새 asset 행. 수집기의 티커→자산 매핑은 활성 행 기준, 내부 참조는 모두 asset_id(UUID)
...
- **disclosure_events**: asset_id, disclosed_at, category, title, url?, dart_rcept_no UNIQUE, severity enum(info, warning, critical), notified_at?. 공시 감시(A6.11)
- **push_subscriptions**: user_id, endpoint UNIQUE, p256dh, auth, user_agent?, last_used_at?. 웹 푸시 구독(A5.8) — 브라우저/기기별 1행, 발송 410 응답 시 삭제
- **notification_settings**: user_id, event_type, channel enum(push, email), enabled. UNIQUE(user_id, event_type, channel). 알림 채널 관리(A5.8)
- **watchlist_items**: user_id, asset_id, kind enum(watch, exclude), note?. UNIQUE(user_id, asset_id). 관심/제외 목록(A5.2) — 공시 감시 대상(A6.11)과 전략의 제외 필터(A6.1)가 참조
...
- **절전 해제 필수**: 크론이 KST 05:00~17:45에 분포(D6). `sudo pmset -a sleep 0` (디스플레이 잠자기는 무관). Mac이 자면 arq 크론은 돌지 않는다.
...
- **복구 필수물은 덤프만이 아니다**: `.env`(특히 `ENCRYPTION_MASTER_KEY`, B4.8)가 없으면 DB 안의 브로커/LLM 키 암호문을 영구히 복호화할 수 없다. `.env` 사본을 덤프와 **분리된 안전한 오프박스 위치**(예: 패스워드 매니저)에 보관하고, 복구 리허설은 "덤프 + .env로 완전 복구"를 기준으로 한다.
...
| KST | 잡 | 내용 |
|---|---|---|
| 08:00 | collect_disclosures_am | 직전 수집 이후 접수 공시 수집 + 중요 공시 즉시 알림 — 후보 만료(08:30) 전 (A6.11) |
...
| 17:35 | collect_disclosures_pm | 보유·관심 종목 당일 공시 수집 + 중요 공시 알림 (A6.11) |
| 17:40 | paper_signal / live_signal | 전략 실행 → 후보 생성 |
| 17:45 | send_daily_briefing | 일일 브리핑 발송 (A6.11) + 외부 heartbeat ping (D7) |
...
**미실행 캐치업**: arq 크론은 지나간 스케줄을 소급 실행하지 않는다 — 재부팅·정전이 수집 창(16:30~17:45)과 겹치면 그날 파이프라인이 통째로 빠질 수 있다(fail-closed라 위험하진 않지만 신호·브리핑이 사라진다). 워커는 기동 시 job_runs로 당일 미실행 잡을 감지해 알림을 보내고, 오너는 헬스 대시보드(A5.9)에서 수동 재실행한다. 자동 소급 실행은 하지 않는다 — 재기동 시각에 따라 안전하지 않을 수 있어 오너 판단에 맡긴다.

## D7. 모니터링

- 1차: **앱 자체 알림(A5.8)** — 잡 실패·정지 이벤트를 푸시/이메일로.
- 2차: 헬스 대시보드(job_runs, 데이터 신선도, LLM 비용).
- 3차: **외부 heartbeat (dead-man's switch)** — `send_daily_briefing` 성공 시 `HEARTBEAT_URL`(예: healthchecks.io)로 ping. 앱 알림은 앱이 살아 있어야 발송된다 — 맥미니/Docker/워커 전체가 죽으면 알림도 함께 침묵하므로, ping 부재를 외부 서비스가 감지해 메일을 보내는 이 장치가 무인 운영의 마지막 안전망이다. Phase 7 도입, 실거래(Phase 9) 전 활성 필수.
...
| 0009 | 정보 모니터링(공시 감시·일일 브리핑) MVP 편입 | 제품 목적의 2축(투자 정보 수집·모니터링) 반영. 비정형 정보는 점수·주문에 자동 개입 금지(A6.11) |
| 0010 | 용어 도움말은 검수된 정적 콘텐츠 — LLM 실시간 생성 금지 | 오너는 투자 초보(A6.12). 틀린 설명은 무설명보다 해롭다. 정적 파일은 git 검수 가능·비용 0. LLM은 초안 제작에만(수록 전 오너 검수) |
| 0011 | ETF는 AI 점수 산출·랭킹 제외 | 재무 팩터 부재로 점수 왜곡(A6.1). ETF 전략은 기술적 조건만 사용 — 결측 보류 규칙에 암묵 의존하지 않고 명시 |
...
| 11 | 비중 복원(밴드 리밸런싱) 임계값 | Phase 12 — 목표 비중 대비 상대 ±25%(5/25 룰) 시드, 회전율·세금 대비 효과를 백테스트로 검증 |
| 12 | 중요 공시 화이트리스트 범위 | Phase 6 확정 — 시드는 A6.11, 설정 테이블로 운영 중 조정 |
| 13 | 모의투자 현금 배당 미반영 | 수익률 과소 편향(인지된 한계). 백테스트는 수정주가로 배당 반영, 모의는 미반영 — 수정주가 재적재 감지(C2)와 연계해 Phase 12 검토 |
| 14 | 용어집 초기 수록 범위·문구 검수 | Phase 3 확정 — 시드는 SoT 등장 용어 전수(약 60~80항목, A6.12), 이후 새 화면 DoD 규칙으로 증분 |
| 15 | 벤치마크 총수익지수(TR) 전환 | 가격지수의 배당 미포함 편향(A6.7.3) — TR 소스 확보 시 전환, 그전까지 결과 화면 고지 |
| 16 | 상폐 종목 과거 시세 확보 | 무료 소스 한계(C2 forward 정책) — 신규 구간은 축적으로 해소, 과거 구간은 유료 데이터 검토 또는 편향 고지 유지 |
